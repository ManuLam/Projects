...
int storeZ (int[] x, int y1, int y2, int z1, int z2) {            // Refreshes the array storing and stores to Z 3x3 matrix
    ...
    for(int j=z1; j<=z2; j++) {
       x[n] = array[i][j];
  //  print(x[n]+" ");   // To show the whole array of Z1 aka [ 5 3 0
  //                                                             6 0 0
...
void creating2 (int x, int y, int l) {        // Creates all the numbers on the screen ( WITH AN EXCEPTION)

  array[x][y] = distinctTerms.get((int)random(2));
